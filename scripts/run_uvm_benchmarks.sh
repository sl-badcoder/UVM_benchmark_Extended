...
PATHFINDER_ROWS, PATHFINDER_PYRAMID, KEEP_GOING.
...
printf "benchmark\tsize\trepeat\tstatus\tlog\n" > "$status_file"
...
run_step() {
  ...
  local workdir="$4"
  shift 4
  local log="$OUT_DIR/${benchmark}_${size//[:\/]/_}_run${repeat}.log"

  echo "[$benchmark] size=$size repeat=$repeat compiling"
  if ! (cd "$workdir" && make clean && "$@") > "$log" 2>&1; then
    printf "%s\t%s\t%s\tcompile_failed\t%s\n" "$benchmark" "$size" "$repeat" "$log" >> "$status_file"
    echo "[$benchmark] compile failed, see $log" >&2
...
run_command() {
  ...
  echo "[$benchmark] size=$size repeat=$repeat running"
  if (cd "$workdir" && "$@") >> "$log" 2>&1; then
    printf "%s\t%s\t%s\tok\t%s\n" "$benchmark" "$size" "$repeat" "$log" >> "$status_file"
  else
    printf "%s\t%s\t%s\trun_failed\t%s\n" "$benchmark" "$size" "$repeat" "$log" >> "$status_file"
    echo "[$benchmark] run failed, see $log" >&2
